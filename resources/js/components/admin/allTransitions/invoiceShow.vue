    ...
    <v-container class="ma-1 grey lighten-2 pa-1" fluid>
        <v-card class="px-5 boxborder mt-5">
            ...
            <v-card-text>
                ...
                <v-btn class="ma-3" color="success" dark to="../allTransitions">
                    بازگشت به لیست
                </v-btn>
...
export default {
    ...
    methods: {
        ...
        getOrder() {
            axios.post('/admin/adminCustomerOrderShow', {"transition_number": this.id})
                .then((data) => {
                    if (data.data) {
        ...
        getCustomerAndSeller() {
            axios.post('/admin/getCustomerAndSeller', {"transition_number": this.id})
                .then((data) => {
                    if (data.data) {
