    ...
    <v-container class="ma-1 grey lighten-2 pa-1" fluid>
        <v-card class="pa-8 boxborder mt-5">
            ...
            <v-card-text class="text-right">
                <h6>
                    در این بخش میتوانید سفارش هایی که برای فروشگاه ارسال کرده اید یا فاکتورهای فروشتان را مشاهده کنید.
                </h6>
                <v-divider></v-divider>
                <v-row>
                    <v-col cols="12" md="6">
        ...
        <v-card v-if="tableType==1" class="mt-3">
            ...
            <v-data-table :headers="headers" :items="orders" :search="search" :sort-desc="true"
                ...
                <template v-slot:[`item.actions`]="{ item }">
                    <v-btn :to="{ path: '/safirShopOrderShow/'+ item.transition_number}" color="primary" dark fab
        ...
        <v-card v-if="tableType==2" class="mt-3">
            ...
            <v-data-table :headers="customerHeaders" :items="customerOrders" :search="search"
                ...
                <template v-slot:[`item.actions`]="{ item }">
                    <v-btn :to="{ path: '/customerOrderShow/'+ item.transition_number}" color="primary" dark fab
                           title="مشاهده"
                           x-small>
                        <v-icon small>mdi-eye</v-icon>
                    </v-btn>
...
<script>
    export default {
        ...
        methods: {
            getAllOrders() {
                axios.post('/getAllSafirShopOrders')
                    .then((data) => {
                        var list = []
                ...
                axios.post('/getAllSafirCustomerOrders')
                    .then((data) => {
                        var list = []
                        for (const [key, value] of Object.entries(data.data)) {
                            list.push(value)
