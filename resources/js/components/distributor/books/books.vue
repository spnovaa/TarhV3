<template>
    <v-container class="ma-1 grey lighten-2 pa-1" fluid>
        <div v-if="profile.dist_active==1">
                            ...
                            <v-card-text>
                                <v-container>
                                    <v-row>
                                        ...
                                        <v-col cols="12" md="4" sm="6">
                                            <v-text-field v-model="editedItem.barcode" disabled
                                                          label="شابک"></v-text-field>
                                        </v-col>
                                        <v-col cols="12" md="4" sm="6">
                                            <v-text-field v-model="editedItem.dist_inv"
                                                          label="موجودی شرکت"></v-text-field>
                                        </v-col>
                                        <v-col cols="12" md="4" sm="6">
                                            <v-text-field v-model="editedItem.dist_disc_percent"
                                                          label="درصد تخفیف"></v-text-field>
                                        </v-col>
                                    </v-row>
                                </v-container>
                            </v-card-text>
        ...
        <div v-if="profile.dist_active==0">
            <v-card>
                <v-card>
    ...
    export default {
        ...
        data: () => ({
            ...
            headers: [
                ...
                {text: 'درصد تخفیف', value: 'pivot.dist_disc_percent'},
        ...
        methods: {
            initialize() {
                axios.post('/distributor/getAllBooks')
                    .then((data) => {
            ...
            save() {
                if (this.editedItem.dist_disc_percent >= 0 && this.editedItem.dist_inv >= 0) {
                    ...
                    alert("لطفا موجودی و تخفیف را به درستی وارد کنید")
                }
            ...
            updateBooks() {
                ...
                axios.post('/distributor/updateBooksList', fd)
                    .then((data) => {
                        if (data.data == "SUCCESS") {
                            ...
                            this.ErrMsg = 'اشکال در ذخیره سازی. لطفا همه فیلدها را بررسی کنید!'
                            this.error = true
                        }
                    })
                    .catch(() => {
