    ...
    <v-container class="ma-1 grey lighten-2 pa-1" fluid>
        ...
        <v-card class="mt-n1"
            ...
            <v-card-text align="right">
                        ...
                        <v-card-text>
                            <v-container>
                                <v-row>
                                    ...
                                    <v-col cols="12" md="4" sm="6">
                                        <v-text-field v-model.number="number" label="تعداد"
                ...
                <!-- <template >
                    <v-btn color="success" dark class="mb-2 mr-2" @click.prevent="updateBooks">
        ...
        <v-card class="pa-8 boxborder mt-10">
                    ...
                    <template v-slot:default>
                        ...
                        >
                            ...
                            <td class="text-right"><a role="button" style="cursor:pointer"><img
                                alt="del" src="/images/delete.png"
                                style=" height:25px;" @click="deleteRow(index)"></a></td>
                        </tr>
                        </tbody>
                    </template>
                </v-simple-table>
                ...
                <div v-if="orders.length">
                  ...
                  " value="2"></v-radio>
                    ...
                    <div class="text-right">
                        <h6>درصورتی که موجودی شرکت در کتابی کمتر از سفارش خرید شما باشد، امکان تایید نشدن بخشی یا همه ی
                            سفارش آن کتاب وجود دارد.</h6>
                        <h6 class="mt-3">هزینه ی حمل بار درصورت تعیین توسط شرکت پخش، به رقم مذکور اضافه خواهد شد.</h6>
                        <h6 class="mt-3">اقلامی که خرید یا مرجوعی آن توسط شرکت پخش مورد تایید قرار بگیرد پس از تایید وصو
                            یا ارسال توسط شما، در موجودی انبارتان اعمال میگردد</h6>
                    </div>
                    <template>
...
export default {
    data: () => ({
        ...
        headers: [
            ...
            {text: 'موجودی شرکت', value: 'pivot.dist_inv'},
            {text: 'درصد تخفیف شرکت', value: 'pivot.dist_disc_percent'},
    ...
    methods: {
        initialize() {
            console.log(this.id)
            axios.post('/shop/getDistInventory', {"dist_id": this.id})
                .then((data) => {
                    this.books = Object.values(data.data[0])
                    this.company_name = data.data[1]
                    this.showTable = true
                    this.loading = false
                })
                .catch(() => {
        ...
        save() {
            if (!this.number || this.number == 0)
                ...
                let item = this.books[this.editedIndex]
                let discRate = this.books[this.editedIndex].pivot.dist_disc_percent / 100
                item.number = this.number
                let sum = item.number * item.fee * (1 - discRate)
                let roundedSum = Math.round((sum + Number.EPSILON) * 100) / 100
                item.singleOrderSum = roundedSum
                this.ordersSum = this.ordersSum + item.singleOrderSum
        ...
        deleteRow(index) {
            ...
            let newSum = 0
            this.orders.forEach(function calcSum(element) {
                let discRate = (1 - (element.pivot.dist_disc_percent / 100))
                newSum += element.fee * element.number * discRate
            });
            this.ordersSum = Math.round((newSum + Number.EPSILON) * 100) / 100
            this.ordersSumNormal = this.ordersSum.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
        }
    },
