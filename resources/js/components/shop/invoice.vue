    ...
    <v-container class="ma-1 grey lighten-2 pa-1" fluid>
        ...
        <v-card class="pa-8 boxborder mt-5 mb-3">
            ...
            <v-card-text>
                <v-row>
                    <v-col cols="6" md="4">
                        <v-text-field v-model="searchCustomer" :class="{err : idErr}" :disabled="inquired" dense
                                      label="کدملی یا شماره همراه"
                ...
                <v-row class="text-right">
                    <div v-if="disabledInquire" class="red--text">
                        <span><small>کدملی 10 رقم و شماره همراه 11 رقم است.</small></span></div>
                </v-row>
                <v-row v-if="inquired">
                    <v-col cols="6" md="3">
                        <v-text-field v-model="customer.info.name" :disabled="isRegistered" dense label="نام (اختیاری)"
                                      outlined></v-text-field>
                    </v-col>
                    <v-col cols="6" md="3">
                        <v-text-field v-model="customer.info.last_name" :disabled="isRegistered" dense label="نام خانوادگی (اختیاری)"
                                      outlined></v-text-field>
                    </v-col>
                    <v-col cols="6" md="3">
                        <v-text-field v-model="customer.info.tel" :class="{err : telErr}"
                                      :disabled="isRegistered||searchCustomer.length===11" :hint="telErr ? telCountErr : telHint"
                                      dense
                                      label="شماره موبایل(اجباری)" outlined
                                      @keyup="validateID()"
                        ></v-text-field>
                    </v-col>
                    <v-col cols="6" md="3">
                ...
                <v-row v-if="inquired && isSellCountLimited">
                    <div>
                        ...
                        هر فرد مجاز به خرید مجموعا
                        {{ customer.info.customer_buy_count_limit }}
                        جلد کتاب است. تعداد خریداری شده فرد جستجو شده تاکنون:
                        <span class="red--text">
                ...
                <v-row v-if="inquired && isSellCountLimited">
                    <div>
                        ...
                        هر فرد مجاز به خرید
                        {{ customer.info.customer_buy_count_limit_per_book }}
                        جلد کتاب از هر عنوان است.
                    </div>
                </v-row>
            </v-card-text>
        </v-card>
            ...
            <v-data-table v-if="showTable" :headers="headers" :items="books" :items-per-page="3" :loading="loading"
              ...
              پیشخوان شما
                ...
                <template v-slot:[`item.shop_lib`]="props">
                    <h6 class="text-center">{{ props.item.pivot.shop_lib }}</h6>
                </template>

                <template v-slot:[`item.actions`]="{ item }">
                    <v-icon class="mr-2" color="success" medium @click="editItem(item)">
                        mdi-shopping
                    </v-icon>
                </template>
            </v-data-table>
                ...
                <v-dialog v-model="dialog" max-width="500px">
                    <v-card>
                        ...
                        <v-card-text>
                            <v-container>
                                <v-row>
                                    ...
                                    <v-col cols="12" md="4" sm="6">
                                        <v-text-field v-model.number="number_inv" :rules="countRules" label="طرح"
                                                      type="number"
                                        ></v-text-field>
                                    </v-col>
                                    <v-col cols="12" md="4" sm="6">
                                        <v-text-field v-model.number="number_lib" :rules="countRules" label="فروشگاه"
                                                      type="number"
                                        ></v-text-field>
                                    </v-col>
                                </v-row>
                                <v-row v-if="orderActive && number_lib>books[editedIndex].pivot.shop_lib">
                                    <div class="red--text">
                                        موجودی فروشگاه شما در این کتاب تنها
                                        {{ books[editedIndex].pivot.shop_lib }}
                                        عدد است!
                                    </div>
                                </v-row>
                                <v-row v-if="orderActive && number_inv>books[editedIndex].pivot.shop_inv">
                                    <div class="red--text">
                                        موجودی انبار شما در این کتاب تنها
                                        {{ books[editedIndex].pivot.shop_inv }}
                                        عدد است!
                                    </div>
                                </v-row>
                            </v-container>
                        ...
                        <v-card-actions>
                            ...
                            <v-btn v-if="orderActive" :disabled="(number_inv>books[editedIndex].pivot.shop_inv || number_inv<0
                            || number_lib>books[editedIndex].pivot.shop_lib || number_lib<0 )"
        ...
        <v-card v-if="orders.length" class="pa-8 boxborder mt-10">
            ...
            <v-card-text>
                <v-simple-table>
                    <template v-slot:default>
                        ...
                        >
                            ...
                            <td class="text-right">{{ item.number_inv }}</td>
                            <td class="text-right">{{ item.number_lib }}</td>
                ...
                <div v-if="orders.length">
                    ...
                    <template>
                        <v-btn v-if="!showReturnOption" :disabled="sendDisabled" class="mb-2 mr-2" color="primary"
                               dark
...
export default {
    data: () => ({
        ...
        headers: [
            ...
            {text: 'سامانه', value: 'shop_inv'},
            {text: 'فروشگاه', value: 'shop_lib'},
    ...
    methods: {
        ...
        save() {
            if ((!this.number_inv || this.number_inv == 0) && (!this.number_lib || this.number_lib == 0))
                this.close()
            else {
                if (!this.number_inv)
                    this.number_inv = 0
                ...
                for (const id in this.orders) {
                    if (this.books[this.editedIndex].barcode == this.orders[id].barcode) {
                        this.number_inv = this.number_inv + this.orders[id].number_inv
                        this.number_lib = this.number_lib + this.orders[id].number_lib
                        this.orders.splice(id, 1);
                    }

                }

                this.books[this.editedIndex].number_inv = this.number_inv
                this.books[this.editedIndex].number_lib = this.number_lib
                this.books[this.editedIndex].singleOrderSum =
                    (this.books[this.editedIndex].number_inv +
                        this.books[this.editedIndex].number_lib) * this.books[this.editedIndex].fee
                ...
                this.payment = 0
                for (const id in this.orders) {
                    this.ordersSum = this.ordersSum + this.orders[id].fee * (this.orders[id].number_inv + this.orders[id].number_lib)
                }
                this.discount_raw = parseInt(this.ordersSum * this.customer.info.percentage_setting)

                this.discount = this.discount_raw + parseInt(this.customer.info.used) >= this.customer.info.limit_setting ?
                ...
                this.bought_count += (this.number_inv + this.number_lib)
        ...
        sendOrders() {
            ...
            let isBuyCountValid =
                (this.customer.info.customer_buy_count_limit == 0 ||
                    (this.bought_count + parseInt(this.customer.info.bought_count)) <= this.customer.info.customer_buy_count_limit)

            if (!this.customer.info.tel || !isBuyCountValid || this.customer.info.tel.length !== 11) {
                validated = false
            } else this.processing = true;

            if (validated) {
                this.sendDisabled = true
                axios.post('/shop/sendInvoice', {"orders": this.orders, "customer": this.customer})
                    .then((data) => {
                        if (data.data == "SUCCESS") {
                            ...
                            this.success = true
                            this.customer.info.bought_count += parseInt(this.bought_count)
                        } else if (data.data === "ORDER_LIMIT") {
                            this.ErrMsg = 'مجموع خرید ثبت شده ی این کدملی یا شماره همراه، به ازای هر عنوان، بیش از حد مجاز است!'
                            this.error = true
                ...
                this.error = true
                this.ErrMsg = ' شماره همراه صحیح مشتری الزامیست!'

            } else if (!isBuyCountValid) {
                this.error = true
                this.ErrMsg = "هر فرد تنها مجاز به خرید مجموعا " +
        ...
        deleteRow(index) {
            ...
            let totCount = 0
            for (const id in this.orders) {
                totCount = (this.orders[id].number_inv + this.orders[id].number_lib)
                this.ordersSum = this.ordersSum + this.orders[id].fee * totCount
                this.bought_count += totCount
            }
        ...
        inquire() {
            ...
            if (this.searchCustomer.length === 10 || this.searchCustomer.length === 11) {
                axios.post('/shop/inquire', {"search_customer": this.searchCustomer})
                    .then((data) => {
                        if (data.data === "INCORRECT") {
                            this.error = true
                            this.ErrMsg = 'کد ملی یا شماره همراه وارد شده صحیح نیست!'
                        } else {
                            this.customer = data.data
                            if (this.searchCustomer.length === 11)
                                this.customer.info.tel = this.searchCustomer
                ...
                this.error = true;
                this.ErrMsg = 'لطفا کد ملی 10 رقمی یا شماره همراه 11 رقمی را به درستی وارد کنید'
                this.processing = false
            }
        },
        ...
        validateID() {
            ...
            this.searchCustomer = this.searchCustomer.replace(/[^0-9]/g, '')
            if (this.inquired) {
                this.customer.info.tel = this.customer.info.tel.replace(/[^0-9]/g, '')
                this.customer.info.national_id = this.customer.info.national_id.replace(/[^0-9]/g, '')
            }
            ...
            // }
            // if (this.inquired) {
            //     this.telErr = this.customer.info.tel.length > 11;
            // }
            if (this.searchCustomer.length !== 10 && this.searchCustomer.length !== 11) {
                this.idErr = true;
                this.disabledInquire = true;
                console.log(this.searchCustomer.length)
            } else {
                this.idErr = false;
                this.disabledInquire = false;
            }
